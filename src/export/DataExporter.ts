...
export class DataExporter {
  ...
  private exportToCSV(options: ExportOptions): string {
    ...
    const rows = this.responses.map(response =>
      this.responseToCSVRow(response, options)
    );
  ...
  private exportToObject(options: ExportOptions): SurveyResults {
    return {
      responses: this.responses.map(response =>
        this.filterResponseData(response, options)
      ),
  ...
  private responseToCSVRow(
    response: SurveyResponse,
    options: ExportOptions
  ): string[] {
    const row: string[] = [response.sessionId, response.completed.toString()];
  ...
  private generateSummary(): SurveyResults['summary'] {
    ...
    const completedWithEndTime = this.responses.filter(
      r => r.completed && r.endTime
    );
    const averageTime =
      completedWithEndTime.length > 0
  ...
  public static exportResponsesToCSV(
  ...
  public static exportResponsesToJSON(
    responses: SurveyResponse[],
  ...
  public static exportResponsesToObject(
    responses: SurveyResponse[],
    options: ExportOptions = { format: 'object' }
  ): SurveyResults {
    const exporter = new DataExporter(responses);
