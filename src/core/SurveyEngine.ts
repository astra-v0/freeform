...
export class SurveyEngine {
  ...
  private setupRenderers(): void {
    this.config.questions.forEach(question => {
      ...
      switch (question.type) {
        case 'text':
          renderer = new TextQuestionRenderer(
            this.config.theme!,
            this.container
          );
          break;
        case 'choice':
          renderer = new ChoiceQuestionRenderer(
            this.config.theme!,
            this.container
          );
          break;
        case 'feedback':
          renderer = new FeedbackFormRenderer(
            this.config.theme!,
            this.container
          );
          break;
  ...
  private getCurrentQuestion(): Question | undefined {
    return this.config.questions.find(
      q => q.id === this.currentState.currentQuestionId
    );
  }
  ...
  private renderCurrentQuestion(): void {
    ...
    if (this.currentRenderer) {
      const questionElement = this.currentRenderer.render(
        question,
        currentAnswer
      );
  ...
  private addNavigation(_question: Question): void {
    ...
    if (this.currentState.canGoBack) {
      const backButton = this.createNavigationButton('Back', false, () =>
        this.goBack()
      );
    ...
    const nextButton = this.createNavigationButton(nextButtonText, true, () =>
      this.goNext()
    );
  ...
  private isLastQuestion(): boolean {
    ...
    const remainingQuestions = this.config.questions.filter(
      q =>
  ...
  private getNextQuestionId(
    ...
    const nextQuestion = remainingQuestions.find(
      q => !this.currentState.visitedQuestions.includes(q.id)
    );
  ...
  private evaluateCondition(
    condition: ConditionalQuestion['condition'],
    answerValue: any
  ): boolean {
  ...
  private goBack(): void {
    if (
      !this.currentState.canGoBack ||
      this.currentState.visitedQuestions.length === 0
    ) {
      return;
    }
