...
export class FeedbackFormRenderer extends QuestionRenderer {
  ...
  render(question: FeedbackFormQuestion, currentAnswer?: UserAnswer): HTMLElement {
    ...
    if (question.fields.firstName) {
      const firstNameField = this.createFormField('First Name', 'firstName', 'Enter your first name');
      formContainer.appendChild(firstNameField);
    }
    
    if (question.fields.lastName) {
      const lastNameField = this.createFormField('Last Name', 'lastName', 'Enter your last name');
      formContainer.appendChild(lastNameField);
    }
    
    if (question.fields.email) {
      const emailField = this.createFormField('Email', 'email', 'Enter your email', 'email');
      formContainer.appendChild(emailField);
    }
    
    if (question.fields.company) {
      const companyField = this.createFormField('Company', 'company', 'Enter company name');
  ...
  private createFormField(label: string, fieldName: string, placeholder: string, type: string = 'text'): HTMLElement {
